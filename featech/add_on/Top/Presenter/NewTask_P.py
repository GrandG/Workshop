...
class NewTesk_P(QtWidgets.QDialog, Ui_NewTask):
    ...
    def InitDate(self):
        ...
        self.dateTimeEditStart.setDateTime(QtCore.QDateTime.fromString(now_day, self.dateTimeEditStart.displayFormat()))
        self.dateTimeEditEnd.setDateTime(QtCore.QDateTime.fromString(now_day, self.dateTimeEditEnd.displayFormat()))
        self.dateTimeEditStart.setDate(QtCore.QDate.currentDate())
        self.dateTimeEditEnd.setDate(QtCore.QDate.currentDate())
    ...
    def CheckData(self):
        ...
        if self.dateTimeEditStart.dateTime() < QtCore.QDateTime.currentDateTime():
            QtWidgets.QMessageBox.information(self, ('提示'), ('开始时间早于当前时间！'))
            self.dateTimeEditStart.setFocus()
            return False
        if self.dateTimeEditStart.dateTime() > self.dateTimeEditEnd.dateTime():
            QtWidgets.QMessageBox.information(self, ('提示'), ('完成时间早于开始时间！'))
            self.dateTimeEditEnd.setFocus()
            return False
    ...
    def GetDataFromView(self):
        ...
        data.setdefault('TaskStartDate', '"%s"' % self.dateTimeEditStart.text())
        data.setdefault('TaskEndDate', '"%s"' % self.dateTimeEditEnd.text())
